...
const fetchUsers = async (usersId) => {
  ...
  const res = await fetch('https://api.twitch.tv/helix/users?' + urlSearchParams, {
    headers: { ...twitchAuthHeaders }
  });
  const json = await res.json();
  return json.data;
...
watch(() => props.gameId, (newVal, oldVal) => {
    ...
    <div class="card" v-for="card in cards" :key="card.id">
      ...
      <div class="card__header">
        <div class="card__avatar">
          <a :href="'https://www.twitch.tv/' + card.user_login + '/about'" target="_blank">
        ...
        <div class="card__info">
          ...
          <a :href="'https://www.twitch.tv/' + card.user_login + '/about'" target="_blank">
            <p class="card__auther" :title="card.user.display_name">{{ card.user.display_name }}</p>
          </a>
        </div>
      </div>
    </div>
